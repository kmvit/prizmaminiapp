...
### 3. Комплексный тест полного пайплайна (`test_full_pipeline_50_questions.py`)

**Самый полный тест**, проверяющий весь пайплайн от начала до конца:

- ✅ Регистрация пользователя
- ✅ Начало теста
- ✅ Ответы на **все 50 вопросов**
- ✅ Завершение теста
- ✅ Генерация **бесплатного отчета** (10 вопросов) с проверкой ИИ-анализа
- ✅ Создание и завершение платежа
- ✅ Генерация **премиум отчета** (50 вопросов) с проверкой ИИ-анализа
- ✅ Проверка наличия обоих отчетов
- ✅ Проверка статуса отчетов через API

**Особенности:**
- Проверяет работу с **всеми 50 вопросами**
- Генерирует **оба типа отчетов** (бесплатный и премиум)
- Проверяет наличие **ИИ-анализа** в обоих отчетах (если Perplexity API включен)
- Проверяет корректность размеров и содержимого отчетов
...
```bash
cd tests
python test_full_pipeline_50_questions.py
```

**Примечание:** Этот тест проверяет весь пайплайн и может занять больше времени, особенно если Perplexity API включен (генерация ИИ-анализа).
