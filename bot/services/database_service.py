...
class DatabaseService:
    ...
    async def start_test(self, telegram_id: int) -> User:
        ...
        async with async_session() as session:
            ...
            # Если пользователь уже завершил тест, не начинаем заново
            if user.test_completed:
                from loguru import logger
                logger.info(f"✅ Пользователь {telegram_id} уже завершил тест, не начинаем заново")
                return user
            
            # Если у пользователя есть текущий вопрос, продолжаем с него
            if user.current_question_id:
                from loguru import logger
                logger.info(f"🔄 Пользователь {telegram_id} продолжает тест с вопроса {user.current_question_id}")
                return user
            
            # Получаем первый вопрос для нового теста
