...
class PerplexityAIService:
    ...
    async def _make_api_request(self, messages: List[Dict], is_premium: bool = False, retry_count: int = 3) -> Dict:
        ...
        # Выводим информацию о запросе к ИИ
        print("\n" + "="*80)
        print("🤖 ЗАПРОС К ИИ:")
        print("="*80)
        
        # Показываем структуру conversation
        total_messages = len(messages)
        system_messages = [msg for msg in messages if msg.get("role") == "system"]
        user_messages = [msg for msg in messages if msg.get("role") == "user"]
        assistant_messages = [msg for msg in messages if msg.get("role") == "assistant"]
        
        print(f"📊 Структура conversation:")
        print(f"   - Всего сообщений: {total_messages}")
        print(f"   - System: {len(system_messages)}")
        print(f"   - User: {len(user_messages)}")
        print(f"   - Assistant: {len(assistant_messages)}")
        
        # Показываем, что весь контекст передается
        if len(user_messages) > 1:
            print(f"\n✅ ВАЖНО: Передается ВЕСЬ conversation с контекстом!")
            print(f"   - Первое сообщение (вопросы-ответы): {len(user_messages[0].get('content', ''))} символов")
            print(f"   - Последнее сообщение (текущий промпт): {len(user_messages[-1].get('content', ''))} символов")
            print(f"   - ИИ видит ВСЕ предыдущие ответы и контекст!")
        
        # Выводим последний user message (текущий запрос)
        if user_messages:
            last_user_msg = user_messages[-1].get("content", "")
            # Обрезаем длинные сообщения для читаемости
            preview = last_user_msg[:1000] + "..." if len(last_user_msg) > 1000 else last_user_msg
            print(f"\n📤 Текущий запрос ({len(last_user_msg)} символов):")
            print("-" * 80)
            print(preview)
            print("-" * 80)
            
            # Если это не первый запрос, показываем, что контекст есть
            if len(user_messages) > 1:
                first_user_msg = user_messages[0].get("content", "")
                print(f"\n📋 Контекст (первое сообщение с вопросами-ответами):")
                print(f"   Размер: {len(first_user_msg)} символов")
                preview_context = first_user_msg[:300] + "..." if len(first_user_msg) > 300 else first_user_msg
                print(f"   Превью: {preview_context}")
        
        print(f"\n🔧 Параметры: model={self.model}, max_tokens={max_tokens}, is_premium={is_premium}")
        print("="*80)
            ...
            try:
                async with httpx.AsyncClient(timeout=600.0) as client:  # Увеличиваем timeout до 10 минут
                    ...
                    if "choices" in result and len(result["choices"]) > 0:
                        ...
                        print(f"📥 Ответ ({content_length} символов, finish_reason='{finish_reason}'):")
                        print("-" * 80)
                        # Выводим первые 1000 символов для читаемости
...
Пожалуйста, изучите эти ответы и подтвердите готовность к созданию подробного психологического анализа. 
...
- ЗАПРЕЩЕНО выдумывать примеры или цитаты, которые "мог бы сказать" пользователь
- Если в ответах нет подходящей цитаты - НЕ создавайте пример, просто опишите характеристику без цитаты
- Все цитаты должны быть дословно взяты из ответов выше (в кавычках)

ВАЖНО: В дальнейших ответах обращайтесь к человеку напрямую через "ВЫ", "ВАШИ", "ВАМ" - НЕ используйте слова "пользователь" или "клиент"."""
            })
            ...
            for i, page_type in enumerate(["page3", "page4", "page5"]):
                print(
                    f"\n🔄 Этап {len(results) + 2}: Генерация страницы '{page_names[page_type]}'...")
                print(f"📋 Промпт для страницы '{page_names[page_type]}':")
                print("-" * 80)

                # Добавляем запрос на конкретную страницу
                page_prompt = self._get_page_specific_prompt(page_type)
                # Выводим промпт (первые 500 символов)
                prompt_preview = page_prompt[:500] + "..." if len(page_prompt) > 500 else page_prompt
...
Используйте эти инструкции для всех страниц данного раздела."""
                ...
                for page_num in range(1, page_count + 1):
                    print(f"\n📄 Генерация страницы {page_counter} ({section_name}, стр. {page_num}/{page_count})...")
                    ...
                    print(f"📋 Промпт для страницы {page_counter}:")
                    print("-" * 80)
                    # Выводим промпт (первые 500 символов)
                    prompt_preview = page_prompt[:500] + "..." if len(page_prompt) > 500 else page_prompt
                    print(prompt_preview)
    ...
    async def analyze_premium_responses_optimized(self, user: User, questions: List[Question], answers: List[Answer]) -> Dict:
        ...
        try:
            ...
            # 1️⃣ ЭТАП: Инициализация conversation с базовой ролью и полными вопросами-ответами
            conversation = []
            
            # Устанавливаем базовую роль эксперта-психолога
            conversation.append({
                "role": "system",
                "content": f"""{PremiumPromptsNew.get_base_prompt()}
...
ВАЖНО: Это базовые инструкции. Дополнительные инструкции для конкретных разделов будут предоставлены по мере необходимости."""
            ...
            # Предоставляем ПОЛНЫЕ данные для изучения (50 вопросов)
            conversation.append({
                "role": "user",
                "content": f"""Вот данные для ПЛАТНОГО психологического анализа (50 вопросов):
...
ВАЖНО: В дальнейших ответах обращайтесь к человеку напрямую через "ВЫ", "ВАШИ", "ВАМ" - НЕ используйте слова "пользователь" или "клиент"."""
            ...
            # Получаем подтверждение от ИИ (опционально, можно пропустить для оптимизации)
            print(f"🔄 ИИ изучает 50 ответов...")
            initial_response = await self._make_api_request(conversation, is_premium=True)
            conversation.append({
                "role": "assistant",
                "content": initial_response["content"]
            })
            print(f"📝 Подтверждение получено: {len(initial_response['content'])} символов")
            ...
            page_counter = 1
            total_api_calls = 1  # Первичный запрос
            ...
            for section_key, section_name, page_count in sections:
                ...
                # Создаем промпт с маркерами страниц (теперь с полными вопросами-ответами в conversation)
                section_prompt = self._create_section_prompt_with_markers(
                    section_key, section_name, page_count, user_data
                )
                
                # Добавляем промпт раздела в conversation (контекст сохраняется!)
                conversation.append({
                    "role": "user",
                    "content": section_prompt
                })
                
                # ОДИН запрос на весь раздел (с полным контекстом!)
                start_time = datetime.utcnow()
                response = await self._make_api_request(conversation, is_premium=True)
                
                # Добавляем ответ в conversation для сохранения контекста
                conversation.append({
                    "role": "assistant",
            ...
            for section_key, section_name, page_count in sections:
                ...
                print(f"   {section_name}: {length} символов ({page_count} страниц, ~{avg_per_page:.0f} символов/страница)")
            print(f"   Общий объем: {total_length} символов")
            print(f"   📞 Всего обращений к ИИ: {total_api_calls} (1 первичный + 9 разделов, вместо 74!)")
            print(f"   📄 Всего страниц: 63")
            print(f"   ⚡ ОПТИМИЗАЦИЯ: 87% экономии запросов!")
            print(f"   ✅ КОНТЕКСТ: Все запросы используют полный conversation с вопросами-ответами!")
...
Используйте загруженные инструкции раздела."""
    ...
    def _create_section_prompt_with_markers(self, section_key: str, section_name: str, page_count: int, user_data: str) -> str:
        ...
        # ИСПОЛЬЗУЕМ ПОЛНЫЕ ВОПРОСЫ-ОТВЕТЫ (они уже в conversation, но добавляем напоминание)
        # ВАЖНО: Полные вопросы-ответы уже переданы в первом сообщении conversation
        # Здесь добавляем только напоминание для ясности
        
        prompt = f"""Создайте ПОЛНЫЙ раздел "{section_name}" ({page_count} страниц).

📋 ВАЖНО: Полные вопросы и ответы пользователя (50 вопросов) уже предоставлены в начале conversation выше.
Используйте ТОЛЬКО реальные цитаты из этих ответов. НЕ выдумывайте примеры.
