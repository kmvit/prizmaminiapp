...
window.LoadingPage = {
    ...
    init() {
        ...
        // Останавливаем предыдущие проверки, если они были
    ...
    async startReportGeneration() {
        try {
            ...
            // Проверяем, генерируется ли уже отчет
            if (status.free_report_status && status.free_report_status.status === 'processing') {
                console.log('⏳ Бесплатный отчет уже генерируется, начинаем мониторинг');
            ...
            if (status.is_paid) {
                ...
                // Пользователь не оплатил - запускаем генерацию бесплатного отчета (только если отчет не готов и не генерируется)
                if (!status.free_report_status || (status.free_report_status.status !== 'ready' && status.free_report_status.status !== 'processing')) {
                    ...
                    if (result.status === 'processing' || result.status === 'already_processing') {
                        console.log('⏳ Генерация отчета запущена, начинаем мониторинг');
                        // Начинаем периодическую проверку статуса
                        this.startStatusPolling();
                        return;
                    }
                    ...
                    console.log('⏳ Бесплатный отчет уже генерируется или готов, пропускаем повторную генерацию');
                }
            }
            
            // Начинаем периодическую проверку статуса каждые 5 секунд
            this.startStatusPolling();
            
        } catch (error) {
            console.error('❌ Ошибка при запуске генерации отчета:', error);
            window.TelegramWebApp?.showAlert('Ошибка при генерации отчета');
            // Даже при ошибке начинаем проверку статуса
            this.startStatusPolling();
        }
    },

    /**
     * Запуск периодической проверки статуса отчета
     */
    startStatusPolling() {
        ...
        console.log('🔄 Запущена периодическая проверка статуса отчета (каждые 5 секунд)');
    },
    ...
    stopStatusPolling() {
        if (this.statusCheckInterval) {
            clearInterval(this.statusCheckInterval);
            this.statusCheckInterval = null;
            console.log('⏹️ Остановлена периодическая проверка статуса отчета');
        }
    },
    ...
    async checkReportStatus() {
        try {
            ...
            if (status.available_report && status.available_report.status === 'ready') {
                ...
                // Останавливаем периодическую проверку перед редиректом
                ...
                if (status.available_report.type === 'premium') {
                    ...
                    if (status.premium_report && status.premium_report.status === 'ready' && status.premium_report.report_path && status.premium_report.report_path.endsWith('.pdf')) {
                        ...
                        console.log('⏳ Премиум еще не готов (или нет PDF), продолжаем проверку');
                        // Продолжаем проверку - не останавливаем polling
                        return;
                    }
                } else if (status.available_report.type === 'free') {
                    console.log('🆓 Бесплатный отчет готов, перенаправляем на price-offer');
                    ...
                    console.log('❓ Неизвестный тип отчета, продолжаем проверку');
                    return;
                }
            }
            ...
            if (status.premium_report_status && status.premium_report_status.status === 'processing') {
                console.log('⏳ Премиум отчет генерируется, продолжаем проверку');
                // Не закрываем приложение, продолжаем проверку статуса
                return;
            }
            ...
            if (status.free_report_status && status.free_report_status.status === 'premium_paid') {
                ...
                ApiClient.generatePremiumReport(telegramId).then(() => {
                    console.log('✅ Генерация премиум отчета запущена');
                    // Продолжаем проверку - polling уже работает
                }).catch(error => {
            ...
            if (status.premium_report_status && status.premium_report_status.status === 'payment_required') {
                console.log('💰 Требуется оплата для премиум отчета, продолжаем проверку');
                // Продолжаем проверку
                return;
            }
            ...
            if (status.free_report_status && status.free_report_status.status === 'processing') {
                console.log('⏳ Бесплатный отчет генерируется, продолжаем проверку');
                // Не закрываем приложение, продолжаем проверку статуса
                return;
            }
            ...
            if (status.available_report && status.available_report.status === 'not_available') {
                console.log('❌ Нет доступного отчета, продолжаем проверку');
                // Продолжаем проверку - возможно отчет еще генерируется
                return;
            }
            ...
            if (status.available_report && status.available_report.status === 'payment_required') {
                console.log('💰 Требуется оплата, продолжаем проверку');
                // Продолжаем проверку
                return;
            }
            ...
            if (status.available_report && status.available_report.status === 'processing') {
                console.log('⏳ Отчет генерируется, продолжаем проверку');
                return;
            }
            
            console.log('📊 Текущий статус отчета (продолжаем проверку):', {
                available_report: status.available_report,
                free_report_status: status.free_report_status,
                premium_report_status: status.premium_report_status
            });
            
        } catch (error) {
            console.error('❌ Ошибка при проверке статуса отчета:', error);
            // Продолжаем проверку - polling уже работает, просто пропускаем эту итерацию
        }
    }
}; 